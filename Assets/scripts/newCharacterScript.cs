...
public class newCharacterScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timer[(int) Timers.Global] < 0)
        {
            inventory.Save(new Vector3(bottleCount, 0, 0));
            window = 1;
        }
    ...
    void OnGUI()
    {
        ...
        {
            Time.timeScale = 0;
            timer[(int) Timers.Global] = 1;
            GUI.Box(new Rect(Screen.width / 2 - 100, Screen.height / 2 - 100, 200, 50), "Бутылок собрано: " + bottleCount 
                + "\n Нажмите ESC/START");
